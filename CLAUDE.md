...
- `TEAMS` / `ISO_MAP` — class fields, default to `{}` and populated from `data.json` in `componentDidMount()`. `TEAMS` is keyed by 3-letter code (e.g. `ARG`, `USA`); fields: `name`, `flag`, `iso2`, `group`, `pos`, `w/d/l`, `gf/ga`, `val`/`vLabel` (squad value), `status` (the stage a team has advanced to — `"R32"`/`"R16"`/`"QF"`/`"SF"`/`"F"` — or `"OUT"` or `"CHAMPION"`), `eliminatedIn` (set only when `status === "OUT"`, the stage they lost in), `games` (group stage), `knockout` (array of `{stage, opp, gf, ga, note?}`, one entry per knockout round played, in order), `nextOpponent` (optional — the side panel's Knockout card shows a "NEXT" chip with this value, falling back to `"TBD"` when absent; populated automatically by `scripts/update-data.js` from the next unfinished knockout match, and deleted again once a team has no currently-scheduled match), `players` (top 3 by value), `facts` (capital/geo/history/society/cuisine). `ISO_MAP` maps numeric TopoJSON country IDs to team codes.
- `state.panelTab` (`'group'` | `'knockout'` | `'value'` | `'facts'`, defaults to `'group'`, reset to `'group'` whenever a new country is selected) — drives which of the side panel's 4 cards is visible. The panel is a tabbed card stack, not one long scroll: Group Stage (matches + W/D/L record), Knockout Stage (played rounds + upcoming-fixture chip), Squad Value + Top 3 Players (merged into one card), Country Facts. Switching tabs calls `showGroupTab`/`showKnockoutTab`/`showValueTab`/`showFactsTab` (each just a `setState({ panelTab: ... })`), and each card plays a `cardIn` keyframe on mount for the switch animation.
...
**Cache gotcha (fixed 2026-07-06):** GitHub Pages serves `data.json` with `Cache-Control: max-age=600`. The fetch call in the bundled template is `fetch('./data.json', { cache: 'no-store' })` specifically to defeat this — without it, a browser tab loaded/refreshed within 10 minutes of a previous fetch silently reused its stale local copy even after the live-update workflow had already pushed newer data (caught when `data.json` had a team's correct `nextOpponent` but the side panel still showed nothing). If this fetch call is ever touched again (e.g. during a future logic edit via the bundle-editing procedure below), keep the `{ cache: 'no-store' }` option.
